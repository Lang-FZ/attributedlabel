...
- (CGSize)sizeThatFits:(CGSize)size {
    ...
    return NISizeOfAttributedStringConstrainedToSize([self mutableAttributedStringWithAdditions], self.frame.size, self.numberOfLines);
}
...
- (void)setText:(NSString *)text {
    ...
    NSMutableAttributedString *attributedString = [[NSMutableAttributedString alloc] initWithString:self.text attributes:@{NSKernAttributeName:@(self.textKern),NSFontAttributeName : self.font}];
    ...
    CGRect rect = [self.text boundingRectWithSize:CGSizeMake(self.frame.size.width, CGFLOAT_MAX) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{
                                                                                                                                                            NSFontAttributeName : self.font,
                                                                                                                                                            NSKernAttributeName : [NSNumber numberWithFloat:_textKern],
...
- (void)_applyLinkStyleWithResults:(NSArray *)results toAttributedString:(NSMutableAttributedString *)attributedString {
    for (NSTextCheckingResult* result in results) {
        if (self.linkColor) {
            [attributedString nimbuskit_setTextColor:self.linkColor range:result.range];
        }
        ...
        if (self.linksHaveUnderlines) {
            [attributedString nimbuskit_setUnderlineStyle:kCTUnderlineStyleSingle
...
- (NSMutableAttributedString *)mutableAttributedStringWithAdditions {
    ...
    if (self.isHighlighted) {
        [attributedString nimbuskit_setTextColor:self.highlightedTextColor];
    }
...
+ (NSMutableAttributedString *)mutableAttributedStringFromLabel:(UILabel *)label {
    ...
    if (label.text.length > 0) {
        ...
        [attributedString nimbuskit_setFont:label.font];
        [attributedString nimbuskit_setTextColor:label.textColor];
